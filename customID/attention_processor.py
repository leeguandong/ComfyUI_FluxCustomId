...
class CATRefFluxAttnProcessor2_0(torch.nn.Module):
    """Attention processor used typically in processing the SD3-like self-attention projections."""

    def __init__(self, hidden_size, cross_attention_dim, dim_head, token_num, eps=1e-6):
        if not hasattr(F, "scaled_dot_product_attention"):
            raise ImportError("FluxAttnProcessor2_0 requires PyTorch 2.0, to use it, please upgrade PyTorch to 2.0.")
        super().__init__()
        ...
        self.to_q_ip = nn.Linear(cross_attention_dim, hidden_size, bias=True)
        ...
        nn.init.zeros_(self.to_q_ip.weight)
        nn.init.zeros_(self.to_q_ip.bias)
        ...
        self.norm_q_ip = RMSNorm(dim_head, eps=eps)
        self.norm_k_ip = RMSNorm(dim_head, eps=eps)
    ...
    def __call__(
        ...
        attention_mask: Optional[torch.FloatTensor] = None,
        image_rotary_emb: Optional[torch.Tensor] = None,
        ip_token: Optional[torch.Tensor] = None,
    ) -> torch.FloatTensor:
        batch_size, _, _ = hidden_states.shape if encoder_hidden_states is None else encoder_hidden_states.shape
        # print(f"CATrRef attn processor: ip_token:{ip_token.shape}, ip_token:{ip_token.mean()}")
        ip_hidden_states = ip_token
        assert ip_hidden_states.shape[0] == hidden_states.shape[0]
        # print(f"ip_hidden_states:{ip_hidden_states.shape}, ip_hidden_states.shape[1]:{ip_hidden_states.shape[1]} self.token_num:{self.token_num}")
        assert ip_hidden_states.shape[1] == self.token_num
        # print(f"ip_hidden_states shape is {ip_hidden_states.shape}")    
        # ip_num = ip_token.shape[1]
        assert hidden_states.ndim==3
        img_path_token_num = hidden_states.shape[1]
        ...
        #ID-adapter #!
        ip_query = self.to_q_ip(ip_hidden_states)
        ...
        ip_query = self.norm_q_ip(ip_query)
        ip_key = self.norm_k_ip(ip_key)
        
        query = torch.cat([query, ip_query], dim=2)
        ...
        if image_rotary_emb is not None:
            from diffusers.models.embeddings import apply_rotary_emb
            # print(query.shape, image_rotary_emb[0].shape, image_rotary_emb[1].shape)
            query = apply_rotary_emb(query, image_rotary_emb)
            key = apply_rotary_emb(key, image_rotary_emb)
        ...
        hidden_states = hidden_states[:, :-self.token_num, :]   #在这里统一截断
        ...
        if encoder_hidden_states is not None:
            ...
            return hidden_states #single block走这里
